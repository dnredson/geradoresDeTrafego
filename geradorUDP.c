...
int main(int argc, char *argv[]) {
    ...
    srand((unsigned)time(NULL)); // Inicializa a semente do gerador randômico
    ...
    while (time(NULL) - start_time < duration) {
        ...
        for (int i = 0; i < num_messages; ++i) {
            uint64_t packet = get_current_time_milliseconds();
            printf("Enviando pacote com timestamp: %" PRIu64 "\n", packet); // Exibe o timestamp
            ssize_t bytes_sent = sendto(client_socket, &packet, sizeof(packet), 0, (struct sockaddr *)&server_address, sizeof(server_address));
